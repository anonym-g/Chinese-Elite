        ...
        try:
            ...
            for new_node in new_data.get('nodes', []):
                ...
                if primary_name in self.name_to_qcode_map:
                    ...
                    existing_node['name'] = self._merge_and_update_names(new_node, qcode, existing_node=existing_node, primary_lang=primary_lang)
                    ...
                    if status in ["REDIRECT", "DISAMBIG"]:
                        logger.warning(f"  - [丢弃] 节点 '{primary_name}' 是一个非简繁重定向，已丢弃。")
                        continue
                    ...
                    if qcode:
                        ...
                        if status == "SIMP_TRAD_REDIRECT" and detail:
                            ...
                            if canonical_name_override:
                                add_title_to_list(f"({api_lang}) {canonical_name_override}" if api_lang != 'zh' else canonical_name_override)
                        ...
                        if qcode in self.master_nodes_map:
                            ...
                            logger.info(f"  - 添加全新节点: '{canonical_name_override or primary_name}' -> {qcode}")
                            new_node['id'] = qcode
                            new_node['name'] = self._merge_and_update_names(new_node, qcode, canonical_name_override=canonical_name_override, primary_lang=primary_lang)
